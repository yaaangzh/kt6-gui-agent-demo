...
检测规则拓扑中的节点框与正交连线。此模式不启动 CodeAgent，不调用外部 API，
...
图片，适合文字清楚、设备 ID 规则、节点框和连线较规整的拓扑图；复杂图标、
曲线、交叉线或低清图片仍应使用 CodeAgent 或 HTTP 视觉服务。识别结果固定为
分析用途：`adapter_id=local-cv-ocr`，`actionable_grounding=false`。
