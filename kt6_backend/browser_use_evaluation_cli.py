"""CLI for one Browser Use + DeepSeek API evaluation repetition."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one isolated Browser Use + DeepSeek API evaluation."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        base_url = args.api_base_url or required_env("KT6_DEEPSEEK_API_BASE_URL")
        model = args.model or required_env("KT6_DEEPSEEK_MODEL")
        api_key = required_env("KT6_DEEPSEEK_API_KEY")
        allowed_hosts = args.api_allowed_host
        if allowed_hosts is None:
            raw_hosts = optional_env("KT6_DEEPSEEK_API_ALLOWED_HOSTS")
