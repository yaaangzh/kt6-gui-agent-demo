...
公共证据归档/评测报告和统一 `.env` 加载已经进入 `main`，并同步到
`eval-current`、`eval-browser-use`、`eval-ui-tars`。根目录 `.env` 由后端和评测 CLI
自动读取，进程已有变量优先；真实 key 不得提交。
...
2026-08-13 `main` 开发环境全量结果为 433 项通过、46 项跳过；三个评测分支的最近
基线见 `test.md`。后续通过、跳过和失败数量仍以当前命令输出为准。跳过项主要来自
开发环境缺少可选 RapidOCR/OpenCV 或实验运行依赖，不是测试失败。
